...
class MediaController extends BaseController {
	...
	public function show($slug)
	{
		...
		if(isset($media)){
			...
			$media_next = Media::where('active', '=', 1)->where('created_at', '>=', date("Y-m-d H:i:s", strtotime($media->created_at)) )->take(Constant::NUMBER_MEDIA_NEXT)->get();
	
			$next_media_count = $media_next->count();
	
			if($next_media_count < Constant::NUMBER_MEDIA_NEXT){
				$get_prev_results = Constant::NUMBER_MEDIA_NEXT - $next_media_count;
