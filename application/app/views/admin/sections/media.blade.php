<link rel="stylesheet" href="{{ URL::to('/') }}/application/assets/admin/js/tagsinput/jquery.tagsinput.css" />
	<div class="col-sm-12" style="padding-left:0px; padding-right:0px;">

		<div class="table-responsive">
			<table class="table table-striped">
				<tr class="table-header">
					<th><a class="ajax" href="media/?sort=title&order=<?php if(isset($_GET['order']) && $_GET['order'] == 'ASC'){ echo 'DESC'; }else{ echo 'ASC'; } ?>">{{ Lang::get('lang.title') }} <i class="fa fa-sort"></i></a></th>
					<th>{{ Lang::get('lang.category') }}</th>
					<th style="min-width:100px;"><a class="ajax" href="media/?sort=active&order=<?php if(isset($_GET['order']) && $_GET['order'] == 'ASC'){ echo 'DESC'; }else{ echo 'ASC'; } ?>">{{ Lang::get('lang.active') }} <i class="fa fa-sort"></i></th>
					
					<th><a class="ajax" href="media/?sort=created_at&order=<?php if(isset($_GET['order']) && $_GET['order'] == 'ASC'){ echo 'DESC'; }else{ echo 'ASC'; } ?>">{{ Lang::get('lang.date_added') }} <i class="fa fa-sort"></i></th>
					<th>{{ Lang::get('lang.actions') }}</th>
					@foreach($media as $item)
					<tr>
						<td><a href="{{ URL::to('media') . '/' . $item->slug }}" data-toggle="modal">
							<?php if(strlen($item->title) > 40){
									echo stripslashes(substr($item->title, 0, 40) . '...');
								  } else {
...
$(document).ready(function(){
	...
	$('a.ajax, .pagination a').click(function(e) {
		e.preventDefault();
	  	var url = $(this).attr('href');
	 	$('section.content').load(url, function(data){
	 		//console.log('hit');
...
<div class="modal-container">
	  ...
	  <div class="modal-dialog" style="min-height:700px;">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
			...
			<input type="hidden" id="redirect" name="redirect" value="{{ URL::to('admin') }}/#/media" />
	      </div>
	      
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">{{ Lang::get('lang.cancel') }}</button>
	        <input type="submit" class="btn btn-success" value="{{ Lang::get('lang.update_media') }}" />
	      </div>
	      {{ Form::close() }}
	...
	<script type="text/javascript" src="{{ URL::to('/') }}/application/assets/admin/js/tagsinput/jquery.tagsinput.js"></script>
	<script>
